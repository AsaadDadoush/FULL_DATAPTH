...
def testbench():
    ...
    rd = Signal(intbv(0)[5:])
    rs1_out = Signal(intbv(0)[5:])
    rs2_out = Signal(intbv(0)[5:])
    clk = Signal(bool(0))
    ...
    def stimulus():
        ...
        print("rs1      | rs2   |   rd     |  rs1_out|     rs2_out  |                data              |   enable  |")
        print("%s    | %s |   %s  |  %s  |    %s     | %s |        %s  | " % \
              (bin(rs1, 5), bin(rs2, 5), bin(rd, 5), bin(rs1_out, 5), bin(rs1_out, 5), bin(data, 32), bin(enable, 1)))
        ...
        print("rs1      | rs2   |   rd     |  rs1_out|     rs2_out  |                data              |   enable  |")
        print("%s    | %s |   %s  |  %s  |    %s     | %s |        %s  | " % \
              (bin(rs1, 5), bin(rs2, 5), bin(rd, 5), bin(rs1_out, 5), bin(rs2_out, 5), bin(data, 32), bin(enable, 1)))
        enable.next = 0
        yield delay(5)
