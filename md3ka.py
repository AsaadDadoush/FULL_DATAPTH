...
def CPU(data, data_in, write_enable, clock, reset):
    dataMem, ALUinput1, ALUinput2, ALU_result, rs1_Out, rs2_Out, jumpInstruction, imm = [
        Signal(intbv(0, min=-2 ** 31, max=2 ** 31)) for i in range(8)]
    ...
    Control_Unit = control(branch, memWrite, memRead, memToReg, ALUOp, immToALU, regWrite, reg1ToPC, pcToALU, opCode,
                           funct3, funct7)
    Adder4 = adder(address, nextInstruction)
    Register_File = RegisterFile(regWrite, rs1_Out, rs2_Out, rs1_In, rs2_In, rd, data, clock)
    ALU_Unit = ALU(ALUinput1, ALUinput2, ALUOp, ALU_result, Zero_Flag)
    Data_Memory = DataMemory(ALU_result, rs2_Out, memRead, memWrite, dataMem, clock)
...
def test_CPU():
    ...
    def simulate():
        ...
        reset.next = 1
        yield delay(10)
        with open('Sort_textCode.txt', 'r') as f:
            for line in f:
