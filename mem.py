...
def memory(addres, data_in, enable, clk, data_out, size):
    memory32 = [Signal(intbv(0)[32:]) for i in range(12287)]

    @always(clk.posedge)
    def write():
        if enable == 1:
            if size == 1:
                memory32[addres].next = data_in[8:0]
            elif size == 2:
                memory32[addres].next = data_in[8:0]
                memory32[addres + 1].next = data_in[16:8]
            elif size == 3:
                memory32[addres].next = data_in[8:0]
                memory32[addres + 1].next = data_in[16:8]
                memory32[addres + 2].next = data_in[24:16]
                memory32[addres + 3].next = data_in[32:24]
        if size == 1:
            data_out.next = intbv(memory32[addres])[32:]
        elif size == 2:
            data_out.next = intbv(concat(memory32[addres + 1], memory32[addres]))[32:]
        elif size == 4:
            data_out.next = concat(memory32[addres + 3], memory32[addres + 2], memory32[addres + 1], memory32[addres])
...
def testbench():
    ...
    def monitor():
        ...
        for i in range(12287):
            addres.next, data_in.next = i, intbv(to_number(program.read(i, 1), 1, True))[8:0]
            yield delay(2)
            print("%s  | %s |   %s  |  %s " % \
        ...
        for i in range(12287):
            ...
            yield delay(2)
            print("%s  | %s |   %s  |  %s " % \
                  (addres + 0, data_in + 0, bin(data_out, 8), bin(enable)))
        addres.next = 0
        yield delay(1)
